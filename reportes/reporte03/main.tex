...
Pruebas automatizadas & Suite incluida y ampliada \\
...
\pendingbox{
Para cerrar la Entrega 3 queda grabar el video de síntesis, incorporar su
enlace en la entrega y realizar la revisión final del PDF generado.
}
...
aplicación simple para servir el tablero y predicciones; empaquetamiento del
modelo como \texttt{modelo-citas}; construcción del paquete instalable y apoyo
en la integración del backend/tablero. \\
...
issues y Pull Requests. El video final debe mantener participación identificable
de los integrantes y reflejar esta distribución de trabajo.

Como preparación adicional para la evaluación de función de cita se versionaron
tres estrategias de \emph{prompting}, el parser común de nueve puntajes y el
pipeline de métricas con Precision, Recall, F1 Macro/Micro y matrices de
confusión. También se completó una primera validación manual por parte del
equipo sobre 20 casos piloto. Este conjunto se conserva como evaluación
provisional hasta completar una segunda validación manual independiente.

Dicha segunda validación se realizó sobre los mismos 20 casos, sin consultar la
primera hoja ni las sugerencias automáticas del piloto. El acuerdo entre ambos
anotadores fue de 12 coincidencias y 8 desacuerdos, es decir una tasa de acuerdo
de 0,60 y un \emph{Cohen Kappa} de 0,4839, valor que corresponde a un acuerdo
moderado y se ubica dentro del rango habitual para taxonomías de función de cita
con nueve clases. Tras reconciliar los desacuerdos se congeló el \emph{Test
Gold} definitivo, que difiere del provisional en un único caso.

Tres limitaciones deben acompañar esa cifra. Primero, el Kappa compara una
anotación asistida con explicaciones contra una independiente, no dos
anotaciones ciegas. Segundo, la reconciliación de los ocho desacuerdos también
fue asistida, y siete de ellos se resolvieron hacia la etiqueta del primer
anotador. Tercero, el conjunto definitivo cubre solo 5 de las 9 categorías
---Background, Application, Evidence, Improvement y Further Reading---, por lo
que el F1 Macro sobre las nueve clases permanece penalizado por las cuatro
ausentes. Ampliar el piloto más allá de 20 casos es la vía para levantar esa
restricción.
...
reordenador. La versión 2 con negativos aleatorios alcanzó Recall@10 de
0,3793 y MRR@10 de 0,2166, mientras que la versión 2 con negativos difíciles
obtuvo 0,2288 y 0,0995, respectivamente. Por esta razón se seleccionó la
versión 2 con negativos aleatorios como modelo final.
...
Para cerrar la Entrega 3 queda pendiente el video de síntesis y la actualización
final de los aportes del equipo. En la extensión de clasificación de función de
cita ya se completó la segunda validación manual independiente y quedó congelado
el \emph{Test Gold} definitivo; resta repetir sobre él las corridas de los
modelos configurados para reportar resultados definitivos. De cara a la continuación del trabajo, el techo del recomendador
está en la primera etapa: con un Recall@100 de 0,5160, la mitad de las consultas
no tiene solución posible por reordenamiento, de modo que el margen de mejora
está en la recuperación de candidatos --por ejemplo mediante representaciones
densas-- antes que en el reordenador.
