...
recomendaciones. Por esta separación, FastAPI y las imágenes Docker no necesitan
entrenar el modelo, copiar \texttt{model-package/} ni descargar el dataset para
servir inferencias.
...
TF--IDF más reordenador lineal, versión 0.2.0, sobre un catálogo de 19.776
artículos.
...
Con Docker Compose, el tablero queda disponible en
\texttt{http://localhost:8080}, la API en
\texttt{http://localhost:8000/api/estado} y Swagger/OpenAPI en
\texttt{http://localhost:8000/docs}. El build instala directamente el wheel
\texttt{modelo-citas} 0.2.0; no ejecuta DVC ni entrenamiento.
...
Validación final del despliegue & Verificada después del refactor \\
...
La suite completa finalizó con 202 pruebas aprobadas. En Windows 11 se habilitó
WSL2, Docker Desktop construyó y levantó los servicios con
\texttt{docker compose up --build}, y el healthcheck de la API respondió
correctamente. El endpoint \texttt{/api/estado} reportó
\texttt{listo=true}, el modelo TF--IDF más reordenador lineal versión 0.2.0 y
19.776 artículos. Desde \texttt{localhost:8080} se cargó un ejemplo real y el
tablero devolvió un ranking de recomendaciones con sus resúmenes. Esta
validación confirma que la arquitectura de producción ya no depende de DVC ni
de reentrenar el modelo durante el build. El procedimiento actualizado se
encuentra en \texttt{docs/manual\_instalacion.md}.

\pendingbox{
Para cerrar la Entrega 3 queda grabar y publicar el video de síntesis e
incorporar su enlace en la entrega. Los manuales y la arquitectura descrita en
este reporte ya fueron actualizados después de la validación final con Docker.
}
...
La contenerización y el despliegue quedaron completados y verificados. Tras el
refactor, la imagen de la API instala el wheel \texttt{modelo-citas} 0.2.0
publicado en S3 y no incorpora dependencias de entrenamiento ni necesita
descargar los datos durante el build. La solución fue validada localmente con
Docker Compose, API saludable y una recomendación real desde el tablero; las
evidencias previas de despliegue en AWS se mantienen como parte de la Entrega 3.

Para cerrar la Entrega 3 queda pendiente el video de síntesis y su enlace final.
En la extensión de clasificación de función de cita ya se realizaron corridas
reales provisionales con Qwen3:8B, Gemini y Cohere. Sin embargo, todavía se
requiere una segunda validación manual independiente, cálculo de acuerdo,
reconciliación y repetición de la evaluación sobre un Test Gold definitivo antes
de presentar esos resultados como definitivos. De cara a la continuación del trabajo, el techo del recomendador
