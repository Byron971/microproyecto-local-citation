...
\subsection{Cambios respecto a la segunda entrega}

\begin{itemize}
    \item \textbf{Segunda versión del reordenador.} El vector de características
    pasó de cinco a diez señales, incorporando metadatos derivados del
    identificador del ACL Anthology. El Recall@10 subió de 0,2882 a 0,3793 y el
    MRR@10 de 0,1490 a 0,2166 sobre las mismas 9.381 consultas de validación.
    \item \textbf{Empaquetamiento del modelo.} El modelo seleccionado se publicó
    como el wheel \texttt{modelo-citas} 0.2.0 en un bucket S3 público. La
    instalación ya no requiere entrenar localmente ni disponer de credenciales de
    AWS.
    \item \textbf{Del proceso único a los contenedores.} En la Entrega 2 el
    tablero ya producía recomendaciones reales, pero servía la línea base TF--IDF
    desde un solo proceso local lanzado con \texttt{uv run tablero}. Ahora la API
    y el tablero son servicios separados, el modelo que se sirve es el reordenador
    empaquetado, y ambos se orquestan con Docker Compose detrás de Nginx.
    \item \textbf{Separación de responsabilidades.} El código de
    experimentación en \texttt{src/} se separó del paquete distribuible en
    \texttt{model-package/}, de modo que la imagen de producción no arrastra
    dependencias de investigación.
    \item \textbf{Extensión de clasificación de función de cita.} Se añadió una
    línea de trabajo que clasifica la función retórica de cada cita en nueve
    categorías, con validación humana por dos anotadores y acuerdo medido.
    \item \textbf{Remoto DVC público.} El remoto por omisión pasó a ser una URL
    HTTPS de solo lectura, para que la recuperación de los datos no dependa de
...
Despliegue en AWS EC2 (post-refactor) & Realizado \\
Validación post-refactor & Local y en la nube \\
Video final & Publicado \\
...
El mismo sistema se desplegó sobre una instancia EC2 de AWS Academy
(\texttt{t2.medium}, Ubuntu 24.04) con la arquitectura posterior al refactor.
Sobre la instancia se instalaron Docker y Compose, se clonó el repositorio y se
levantaron los dos servicios con \texttt{docker compose up -d --build}. El
\emph{security group} expone únicamente el puerto 8080, por el que Nginx sirve
el tablero y enruta las peticiones hacia la API por la red interna de Compose.
No se requieren credenciales de AWS en la instancia: el wheel
\texttt{modelo-citas} 0.2.0 se descarga desde una URL pública de S3.
...
\centering
\includegraphics[width=\linewidth]{despliegue_aws_postrefactor_tablero.png}
\caption*{(a) Tablero servido desde la IP pública de EC2}
\end{minipage}
\hfill
\begin{minipage}{0.42\textwidth}
\centering
\includegraphics[width=\linewidth]{despliegue_aws_postrefactor_contenedores.png}
\caption*{(b) Contenedores activos en la instancia}
\end{minipage}

\caption{Despliegue en la nube posterior al refactor: el tablero responde desde
la IP pública y los dos contenedores permanecen activos, con la API en estado
\emph{healthy}.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.88\linewidth]{despliegue_aws_postrefactor_consola_ec2.png}
\caption{Ficha de la instancia en la consola de EC2. La IP pública 3.235.175.77
coincide con la del tablero y la IP privada 172.31.66.17 con el \emph{prompt} de
la sesión SSH, de modo que las tres evidencias corresponden a la misma máquina.}
\end{figure}

\pendingbox{
Si el equipo decide que la variante v2 debe ser también la versión
...
% El reporte de trabajo en equipo es un entregable aparte de maximo 1 pagina
% segun la rubrica; va en su propia hoja, como en las entregas 1 y 2.
\clearpage
...
Compose y desplegada sobre una instancia EC2 de AWS Academy, con API saludable y
una recomendación real devuelta desde el tablero servido por la IP pública.

El video de síntesis de la Entrega 3 está publicado y disponible en
% ======================================================================
% Enlace del video de sintesis (OneDrive institucional de Uniandes).
% Si al compilar el reporte queda en 11 paginas, comentar la linea de
% \url y descomentar la de \href: muestra un texto corto en lugar de la
% URL completa y recupera las dos lineas que hacen la diferencia.
% ======================================================================
\url{https://uniandes-my.sharepoint.com/:v:/g/personal/c_caicedob_uniandes_edu_co/IQCplaGZmE26Ta5TTGF6J3IjAfeb1JOryNUASjk-G5MmL3U?e=7wzTIj}.
% \href{https://uniandes-my.sharepoint.com/:v:/g/personal/c_caicedob_uniandes_edu_co/IQCplaGZmE26Ta5TTGF6J3IjAfeb1JOryNUASjk-G5MmL3U?e=7wzTIj}{este enlace}.
