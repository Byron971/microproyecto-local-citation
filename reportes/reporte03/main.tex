...
busca reducir ese esfuerzo tomando como punto de partida el fragmento de texto
en el que se necesita incorporar la referencia.
...
repositorio \textit{Local-Citation-Recommendation}. El corpus utilizado contiene
63.768 contextos de cita y 19.776 artículos candidatos. Las particiones
empleadas incluyen 30.390 consultas de entrenamiento, 9.381 de validación y
9.585 de prueba.
...
ordenar esos candidatos tratando de ubicar la cita correcta en las primeras
posiciones.
...
la estrategia utilizada para escoger los negativos.
...
Esto indica que la diferencia entre los experimentos está en el ordenamiento y
no en la recuperación inicial.

El resultado de la estrategia \texttt{hard} también fue útil para entender una
...
características, el reordenador tiene poca información adicional para
...
Para evitar que los resultados quedaran únicamente registrados en las
terminales de cada integrante, el entrenamiento se integró con MLflow. Cada
corrida conserva sus parámetros, métricas de ranking, tiempos de procesamiento
y artefactos asociados.

La configuración se centralizó en
\texttt{src/tracking/mlflow\_setup.py}, de manera que todos los experimentos
utilicen la misma estructura de seguimiento.

A continuación se muestran las dos corridas registradas en MLflow y una de las
comparaciones de tiempo utilizadas durante el análisis.
...
Las siguientes capturas muestran el detalle de métricas y parámetros almacenado
para cada una de las dos ejecuciones.
...
El uso de MLflow permitió comparar los experimentos sobre una misma base y
conservar evidencia de cómo se obtuvieron los resultados utilizados para
...
Uno de los inconvenientes de AWS Academy es que la dirección IP pública puede
cambiar al detener y volver a iniciar la instancia. Para evitar tener que
actualizar esta dirección manualmente cada vez, se creó el script
...
Otro avance importante de esta entrega fue convertir el recomendador en una
librería Python instalable. El paquete se encuentra en
\texttt{model-package/} y se distribuye con el nombre
\texttt{modelo-citas}. La versión utilizada actualmente es la 0.1.0.

El wheel incluye el recuperador TF--IDF, el reordenador lineal, la
configuración, el modelo entrenado y la información necesaria para devolver
...
del repositorio, se construyó el wheel y se instaló en un entorno virtual limpio.
Desde ese entorno se importó \texttt{modelo\_citas} directamente desde
\texttt{site-packages} y se ejecutó una predicción real.

También se ejecutaron las pruebas específicas del paquete mediante Tox y,
después de integrarlo con la rama principal, se volvió a ejecutar la suite
...
El paquete \texttt{modelo-citas} se integró posteriormente con un backend
desarrollado en FastAPI. Al iniciar la aplicación, el backend carga el modelo
empaquetado y queda disponible para responder las consultas enviadas desde la
API o desde el tablero.
...
estudio de los datos, desempeño del modelo y diagnóstico de negativos. De esta
forma, el prototipo reúne tanto la funcionalidad de recomendación como parte de
la información utilizada durante el desarrollo del modelo.
...
El flujo para utilizar el tablero es sencillo:
...
Hasta el momento se han completado los componentes asociados con datos,
experimentación, selección del modelo, empaquetamiento e integración local con
la API y el tablero.
...
\texttt{model-package}. Antes de aprobarlo se comprobó el entrenamiento, las
pruebas del paquete, la construcción del wheel, su instalación en un entorno
...
En términos de ingeniería, el proyecto avanzó desde un flujo principalmente
experimental hacia una solución más reutilizable. Los datos están versionados
con DVC, los experimentos quedan registrados en MLflow, el modelo puede
instalarse como wheel y ya es consumido por FastAPI y por el tablero.
