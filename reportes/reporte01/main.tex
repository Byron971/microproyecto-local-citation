...
\begin{tabular}{rl}
\textbf{Nombres participantes:} & Carlos Duque, John Arias, Carlos Caicedo, Jose Zambrana \\[4pt]
...
% ======================= REPOSITORIOS GIT Y DVC =======================
\section{Repositorios Git y DVC}

Con el fin de garantizar la trazabilidad y reproducibilidad del proyecto, se establecieron dos repositorios independientes: uno para el versionamiento del código fuente y otro para el versionamiento de los datos.
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{buckets3.jpeg}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{creaciondvcs3.jpeg}
\caption{Proceso de carga (\textit{push}) de los datos versionados al repositorio DVC en S3}
\label{fig:dvc2}
\end{figure}
% ======================= 6. FORMULACIÓN DEL PROBLEMA =======================
\section{Formulación del problema de aprendizaje automático}
...
% ======================= 7. ENFOQUE PREDICTIVO =======================
\section{Enfoque predictivo}

La solución se desarrollará de manera incremental. Como línea base se utilizará una representación TF-IDF con similitud coseno, que permitirá establecer un punto de comparación sencillo. Posteriormente se evaluará un modelo supervisado, inicialmente una regresión logística o un clasificador lineal, utilizando características derivadas de la relación entre el contexto y el artículo candidato.

Entre las variables candidatas se encuentran la similitud TF-IDF entre el contexto y el título, la similitud entre el contexto y el \textit{abstract}, coincidencias de términos relevantes y características básicas de longitud. Si el tiempo y los recursos lo permiten, se evaluarán representaciones semánticas basadas en \textit{embeddings} de textos científicos. El enfoque incremental facilitará comparar alternativas antes de seleccionar el modelo que será empaquetado y desplegado.

% ======================= 8. RESULTADO ESPERADO DEL MODELO =======================
\section{Resultado esperado del modelo}

Para un contexto de entrada, el sistema devolverá una lista ordenada de artículos con un puntaje de relevancia. Un ejemplo conceptual de salida se presenta en la Tabla~\ref{tab:ranking}.

\begin{table}[h!]
\centering
\caption{Ejemplo conceptual de salida del modelo}
\label{tab:ranking}
\rowcolors{2}{lightgray}{white}
\begin{tabular}{c l c}
\toprule
\textbf{Posición} & \textbf{Artículo candidato} & \textbf{Puntaje de relevancia} \\
\midrule
1 & Artículo A & 0.91 \\
2 & Artículo B & 0.84 \\
3 & Artículo C & 0.76 \\
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.64\textwidth]{eda_particiones.png}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.92\textwidth]{eda_longitudes.png}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.76\textwidth]{eda_similitud_tfidf.png}
...
% ======================= 10. MAQUETA DEL PROTOTIPO =======================
\section{Maqueta inicial del prototipo}
...
\begin{tikzpicture}[font=\sffamily\small]
  ...
  \node[anchor=north west, color=primaryblue, font=\bfseries\footnotesize, text width=5.8cm] at (0.6,6.3)
  ...
  \draw[draw=midgray, fill=lightgray!60, rounded corners=2pt] (7.1,1.2) rectangle (12.4,4.6);
...
El equipo distribuyó el avance entre la definición del producto, la documentación, la infraestructura reproducible y la exploración. La evidencia verificable disponible en el historial de Git se resume en la Tabla~\ref{tab:equipo}. Antes de la entrega, cada integrante debe confirmar que su aporte individual esté reflejado mediante commits propios, tal como exige la guía del curso.

\begin{table}[h!]
\centering
\caption{Aportes observables en el repositorio al 22 de agosto de 2026}
\label{tab:equipo}
\small
\begin{tabular}{p{3.1cm} p{8.8cm}}
\toprule
\textbf{Integrante} & \textbf{Aporte o evidencia registrada} \\
\midrule
John Arias & Inicialización de Git y DVC, configuración del remoto S3, versiones de datos y desarrollo del notebook de exploración. \\
Carlos Duque & Documentación y actualización del archivo \texttt{README.md}. \\
Jose Zambrana & Configuración del remoto del repositorio e integración del reporte de la Entrega 1. \\
Carlos Caicedo & Aporte pendiente de identificar y respaldar con commits en el repositorio. \\
\bottomrule
\end{tabular}
\end{table}
