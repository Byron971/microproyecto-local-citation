...
El equipo dispone de un servidor MLflow desplegado sobre una instancia EC2 (Ubuntu 24.04, \texttt{t2.medium}), accesible en el puerto 8050. La configuración está centralizada en \path{src/tracking/mlflow_setup.py}, de modo que todos los experimentos comparten el mismo nombre y las mismas claves de métricas, condición necesaria para que las comparaciones sean válidas. Las corridas supervisadas completas se registraron primero en el almacén local reproducible; antes de la entrega deben repetirse con \path{MLFLOW_TRACKING_URI} apuntando al servidor para centralizar sus artefactos y actualizar la evidencia visual.

\begin{figure}[h!]
\centering
\includegraphics[width=0.92\textwidth]{mlflow_tabla_runs.png}
\caption{Corridas de la línea base registradas en el servidor MLflow de EC2. La captura debe actualizarse después de ejecutar allí las dos variantes supervisadas.}
\label{fig:runs}
\end{figure}
