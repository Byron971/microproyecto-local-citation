...
José Zambrana &
Responsable del entrenamiento y la evaluación del modelo supervisado de relevancia sobre los pares generados (\textit{issue} \#6). \emph{Sin \textit{commits} reportados en el repositorio a la fecha de corte de este reporte.} \\[3pt]

% TODO: idem para los issues #8 y #9 (tablero).
Carlos Duque &
Responsable del desarrollo del tablero conforme a la maqueta aprobada y de su conexión con los resultados del análisis (\textit{issues} \#8 y \#9). \emph{Sin \textit{commits} reportados en el repositorio a la fecha de corte de este reporte.} \\
\bottomrule
\end{tabular}
\end{table}

\vspace{-4pt}
\noindent\footnotesize\textit{Fecha de corte: 1 de septiembre de 2026. La verificación se hizo sobre el historial de la rama principal y de las ramas de trabajo del repositorio, contando únicamente los \textit{commits} autorizados por cada integrante.}\normalsize
